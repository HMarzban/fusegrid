...
- **`main.js`'s line pin rises to 803 exactly once**, in **Task 3** — this plan's
  only `main.js`-touching task. Additions total **+11** over R8's 793. The
...
```js
  ...
  // and coach_dismissed emits; ro.coach2) — bumped 793->803. The v2 transition
  // itself lives in src/app/coach.js precisely so this gate keeps biting.
  check("main.js stays a lean browser entry (<=803 lines)",
    L.length<=803,String(L.length));
```

If your `main.js` measures above 803, move more of the transition into
