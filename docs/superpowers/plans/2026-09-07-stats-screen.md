...
- **`main.js`'s line pin rises to 770 exactly once**, in **Task 2** — this
  plan's first `main.js`-touching task. Additions total **+16** over R1's 754.
  If your implementation lands above 770, move the excess into
...
```js
  ...
  // run_end/score_set/plaque_unlock edges) — bumped 754->770.
  check("main.js stays a lean browser entry (<=770 lines)",
    L.length<=770,String(L.length));
```
...
above 770, move the excess into `src/app/stats.js` — **never raise the pin**.
