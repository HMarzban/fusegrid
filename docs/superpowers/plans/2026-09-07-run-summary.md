...
- **`main.js`'s line pin rises to 760 exactly once**, in Task 3 (this plan's only
  ...
  above 760, move the excess into `src/app/bests.js` — **never raise the pin**.
...
```js
  ...
  // the feedTally line; ro.run) — bumped 733->760.
  check("main.js stays a lean browser entry (<=760 lines)",
    L.length<=760,String(L.length));
```

If your `main.js` measures above 760, move the excess into `src/app/bests.js` —
**never raise the pin**.
