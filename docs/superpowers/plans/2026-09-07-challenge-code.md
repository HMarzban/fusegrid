...
- **`main.js`'s line pin rises to 792 exactly once**, in **Task 2** — this plan's
  first `main.js`-touching task. Additions total **+9** over R3's 782.
...
```js
  ...
  // playChallenge boot branch; the KeyB copy block) — bumped 782->792.
  check("main.js stays a lean browser entry (<=792 lines)",
    L.length<=792,String(L.length));
```
