...
const HUD_TEXT="#dfe7f5", HUD_MUTED="#7385ad",
  HUD_PANEL="rgba(13,18,32,0.72)", HUD_LINE="#26324a";
export function drawHudChips(c, world){
  ...
  for(let i=0;i<n;i++){
    c.save(); c.translate(16+i*19,25); c.scale(0.62,0.62);
    drawIcon(c,"heart","#ff3b5c",0);
    c.restore();
   }
  if(lives>n){
    c.fillStyle=HUD_TEXT; c.font="900 12px ui-monospace,monospace";
    c.fillText("+"+(lives-n),16+6*19,25);
   }
  const chip=(x,w,label,count,col)=>{
    c.fillStyle=HUD_PANEL; c.fillRect(x,10,w,30);
    c.strokeStyle=HUD_LINE; c.lineWidth=1; c.strokeRect(x+0.5,10.5,w-1,29);
    c.save(); c.translate(x+17,25); c.scale(0.55,0.55);
    drawIcon(c,label==="BOMB"?"bomb":"fire",col,0);
    c.restore();
    c.fillStyle=HUD_MUTED; c.font="9px ui-monospace,monospace";
    c.fillText(label,x+31,18);
    c.fillStyle=HUD_TEXT; c.font="900 13px ui-monospace,monospace";
