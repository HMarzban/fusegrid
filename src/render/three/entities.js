...
const BODY_B=sharedMat(new THREE.MeshPhongMaterial({color:"#15181f",
  shininess:110,specular:new THREE.Color("#ffffff")}));
const RING_COL={power:"#ff4d5e",pierce:"#8f8fff",line:"#ffd447",
  remote:"#9aa3c0"};
const RINGM={}; for(const k in RING_COL)
  RINGM["b_"+k]=sharedMat(new THREE.MeshLambertMaterial({color:RING_COL[k]}));
...
export function createPools(biome, atlas){
  ...
  const helmetMat=sharedMat(new THREE.MeshLambertMaterial({color:"#37f0d0"}));
  const T4=CFG.TILE*0.01;
  const body=new THREE.Mesh(sharedGeo(new THREE.SphereGeometry(
    CFG.TILE*0.26,16,12)),sharedMat(new THREE.MeshLambertMaterial(
    {color:"#f4f7ff"})));
  body.position.y=CFG.TILE*0.30;
  body.castShadow=true; body.receiveShadow=true;
  const helmet=new THREE.Mesh(sharedGeo(new THREE.SphereGeometry(
    CFG.TILE*0.29,16,12,0,Math.PI*2,0,Math.PI/2)),helmetMat);
  helmet.position.y=CFG.TILE*0.34; helmet.castShadow=true;
  const visorMat=new THREE.MeshBasicMaterial({transparent:true});
  if(atlas&&atlas.visor instanceof THREE.Texture){
    visorMat.map=atlas.visor; visorMat.color.set("#ffffff");
   } else visorMat.color.set("#0b1020");
  const visor=new THREE.Mesh(sharedGeo(new THREE.CylinderGeometry(
    CFG.TILE*0.245,CFG.TILE*0.245,CFG.TILE*0.11,16,1,true,
    -Math.PI*0.55,Math.PI*1.1)),visorMat);
  ...
  const rod=new THREE.Mesh(sharedGeo(new THREE.CylinderGeometry(T4*0.9,
    T4*0.9,CFG.TILE*0.14,6)),rodMat);
  rod.position.y=CFG.TILE*0.70;
  ...
  function eyeFor(t){
    let m=eyeMats[t];
    if(!m){ m=new THREE.MeshBasicMaterial({transparent:true});
      if(atlas&&atlas["eye_"+t] instanceof THREE.Texture){
        m.map=atlas["eye_"+t]; m.color.set("#ffffff"); }
  ...
  const bombBodyGeo=sharedGeo(new THREE.SphereGeometry(CFG.TILE*0.30,16,12));
  const fuseGeo=sharedGeo(new THREE.CylinderGeometry(1.5,1.8,11,6));
  const sparkGeo=sharedGeo(new THREE.SphereGeometry(2.7,8,6));
  const vringGeo=sharedGeo((()=>{ const g=new THREE.TorusGeometry(
    CFG.TILE*0.315,1.6,8,24); g.rotateX(Math.PI/2); return g; })());
  ...
  for(let i=0;i<POOL_CAPS.bombs;i++){
    ...
    const f=new THREE.Mesh(fuseGeo,fuseMat); f.rotation.z=0.35;
    f.position.y=CFG.TILE*0.72;
    const sp=new THREE.Mesh(sparkGeo,SPARK_A); sp.position.y=CFG.TILE*0.84;
    const rg=new THREE.Mesh(vringGeo,RINGM.b_power); rg.visible=false;
    rg.position.y=3;
    const cap=new THREE.Mesh(capGeo,capMatB); cap.position.y=CFG.TILE*0.63;
    s.add(b,f,sp,rg,cap); s.visible=false;
  ...
  const pickGeo=sharedGeo(new THREE.BoxGeometry(CFG.TILE*0.44,
    CFG.TILE*0.44,CFG.TILE*0.44));
  const iringGeo=sharedGeo((()=>{ const g=new THREE.RingGeometry(
    CFG.TILE*0.30,CFG.TILE*0.46,20); g.rotateX(-Math.PI/2); return g; })());
  const itemMats={}, itemRings={};
  function matForItem(t,col){
    let m=itemMats[t];
    if(!m){ m=sharedMat(new THREE.MeshLambertMaterial());
  ...
  function ringForItem(t,col){
    let m=itemRings[t];
    if(!m){ m=sharedMat(new THREE.MeshBasicMaterial({transparent:true,
        opacity:0.4,depthWrite:false,
        blending:THREE.AdditiveBlending}));
      m.color.set(col||"#ffffff"); itemRings[t]=m; }
    return m;
   }
  ...
  function crossedQuads(sz){
    const a=new THREE.PlaneGeometry(sz,sz), b=new THREE.PlaneGeometry(sz,sz);
    b.rotateY(Math.PI/2);
    const g=new THREE.BufferGeometry(), n=a.attributes.position.count;
    g.setAttribute("position",new THREE.Float32BufferAttribute([
      ...Array.from(a.attributes.position.array),
      ...Array.from(b.attributes.position.array)],3));
    g.setAttribute("normal",new THREE.Float32BufferAttribute([
  ...
  const bladeMat=new THREE.MeshBasicMaterial({transparent:true,opacity:0.55,
    depthWrite:false,side:THREE.DoubleSide,
    blending:THREE.AdditiveBlending});
  if(atlas&&atlas.fire instanceof THREE.Texture){ bladeMat.map=atlas.fire;
  ...
  group.add(blades);
  const coreGeo=new THREE.BoxGeometry(CFG.TILE*0.40,12,CFG.TILE*0.40);
  const coreMat=new THREE.MeshBasicMaterial({color:"#fff3b0",transparent:true,
  ...
  function update(world){
    ...
    for(let i=0;i<its.length&&ii<POOL_CAPS.items;i++){
      ...
      const pk=s.children[0], rg=s.children[1];
      ...
      rm.opacity=0.30+0.22*Math.sin(5*t);
    ...
    bladeMat.opacity=n>0?Math.max(0,maxSc*(0.55+0.45*Math.sin(t*24))):0;
