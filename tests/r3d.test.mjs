...
//    sx offsets in [-260,300], sy offsets in [0,280]; PAD margins all round
{
  let minX = Infinity,
    ...
    minY = Infinity,
    maxY = -Infinity;
  for (let gx = 0; gx <= CFG.COLS; gx++)
    for (let gy = 0; gy <= CFG.ROWS; gy++) {
  ...
  const sxLo = minX - PROJ.OFF_X,
    sxHi = maxX - PROJ.OFF_X;
  const syLo = minY - PROJ.OFF_Y,
    syHi = maxY - PROJ.OFF_Y;
  check(
    "bbox sx extents == [-260,+300]",
    sxLo === -260 && sxHi === 300,
    sxLo + ".." + sxHi,
  );
  check(
    "bbox sy extents == [0,+280]",
    syLo === 0 && syHi === 280,
    syLo + ".." + syHi,
  );
  ...
  check(
    "right margin == PAD",
    PROJ.canvasW - maxX === PROJ.PAD,
    PROJ.canvasW - maxX + "",
  );
  check(
    "bottom margin == PAD",
    PROJ.canvasH - maxY === PROJ.PAD,
    PROJ.canvasH - maxY + "",
  );
  check(
    "top wall top == PAD",
    minY - PROJ.WALL_H === PROJ.PAD,
    minY - PROJ.WALL_H + "",
  );
}
...
// 3) sy is monotonic in gx+gy (diagonal scan over all tile corners)
{
  ...
  for (let s = 0; s <= CFG.COLS + CFG.ROWS; s++)
    for (
      let gx = Math.max(0, s - CFG.ROWS);
      gx <= Math.min(CFG.COLS, s);
      gx++
    ) {
...
// 4) margin equations: top wall never clipped, bottom margin exactly PAD
{
  ...
  check(
    "OFF_Y >= WALL_H (top not clipped)",
    PROJ.OFF_Y >= PROJ.WALL_H,
    PROJ.OFF_Y + ">=" + PROJ.WALL_H,
  );
  check(
    "canvasH - ((COLS+ROWS)*TILE_H/2 + OFF_Y) == PAD",
...
initFx();
{
  ...
  getFx().push(
    { x: 320, y: 240, t: 0.1, life: 1, color: "#ffffff", size: 4 },
  ...
  for (let y = 0; y < CFG.ROWS; y++)
    for (let x = 0; x < CFG.COLS; x++) {
      ...
      if (t === T.WALL) walls++;
      else if (t === T.BRICK) bricks++;
    }
  const liveIt = w.items.filter((i) => !i.taken && !i.buried).length;
  ...
  check(
    "195 floor painters (tier FLOOR)",
    cnt(TIERS.FLOOR) === 195 && cnt(TIERS.FLOOR) === CFG.COLS * CFG.ROWS,
    cnt(0) + "",
  );
  {
    ...
    const onlyWalls = { ...w.grid },
      onlyBricks = { ...w.grid };
    for (const k in onlyWalls)
      if (onlyWalls[k] === T.BRICK) onlyWalls[k] = T.EMPTY;
    for (const k in onlyBricks)
      if (onlyBricks[k] === T.WALL) onlyBricks[k] = T.EMPTY;
    const cw = buildPainters({ ...w, grid: onlyWalls }).filter(
      (p) => p.tier === TIERS.BLOCK,
    ).length;
    const cb = buildPainters({ ...w, grid: onlyBricks }).filter(
      (p) => p.tier === TIERS.BLOCK,
    ).length;
    check(
      "every wall painter (tier BLOCK, walls-only grid)",
      cw === walls,
      cw + " vs " + walls,
    );
    check(
      "every brick painter (tier BLOCK, bricks-only grid)",
      cb === bricks,
      cb + " vs " + bricks,
    );
  }
  check(
    "every blade tile painter (tier BLADE)",
    cnt(TIERS.BLADE) === bladeTiles,
    cnt(TIERS.BLADE) + " vs " + bladeTiles,
  );
  check(
    "live items+bombs+enemies+player+fx painters (tier ENTITY)",
    cnt(TIERS.ENTITY) === liveIt + w.bombs.length + liveEn + 1 + getFx().length,
    cnt(TIERS.ENTITY) +
      " vs " +
      (liveIt + w.bombs.length + liveEn + 1 + getFx().length),
  );
}
...
// 6) liveness exclusions + per-kind presence (synthetic world)
{
  ...
  const w = {
    ...
    items: [
      { x: 60, y: 60, t: "fire", col: "#ff5d73", taken: false },
      { x: 200, y: 60, t: "heart", col: "#ff5d73", taken: true },
    ],
  };
  let ps = buildPainters(w);
  const has = (depth, tier, list) =>
    list.some((p) => p.depth === depth && p.tier === tier);
  ...
  check(
    "both blade tiles present (d5,d6 tBLADE)",
...
// 7) sort semantics: (depth,tier), occlusion at equal depth
{
  ...
  });
  {
    ...
    check(
      "(depth,tier) sort independent of input order",
    ...
    for (let i = 1; i < s1.length; i++) {
      const a = s1[i - 1],
        b = s1[i];
      if (b.depth < a.depth || (b.depth === a.depth && b.tier < a.tier))
        mono = false;
    }
  ...
  }
  {
    ...
    const idx = (depth, tier) =>
      s.findIndex((p) => p.depth === depth && p.tier === tier);
    check(
      "front wall (d14) sorts after back entity (d6)",
      idx(6, TIERS.ENTITY) < idx(14, TIERS.BLOCK),
      idx(6, TIERS.ENTITY) + " < " + idx(14, TIERS.BLOCK),
    );
    check(
      "behind-wall entity at EQUAL depth sorts before wall (occluded)",
...
// 8) shade(): channel-wise Math.round(v*factor), darker output
{
  ...
  check(
    'shade("#ffffff",0.7) -> darker "rgb(r,g,b)"',
    !!m && +m[1] === exp && +m[2] === exp && +m[3] === exp && +m[1] < 255,
    s,
  );
  check(
    'shade("#ff8000",0.5) -> "rgb(' +
      Math.round(255 * 0.5) +
      "," +
      Math.round(128 * 0.5) +
      ',0)"',
    shade("#ff8000", 0.5) ===
      "rgb(" + Math.round(255 * 0.5) + "," + Math.round(128 * 0.5) + ",0)",
    shade("#ff8000", 0.5),
  );
}
...
//     project(2.5,2.5) = {sx:(2.5-2.5)*20+284=284, sy:(2.5+2.5)*10+48=98}
{
  ...
  check(
    "blade painter billboards at project(tx+.5,ty+.5) (exactly once)",
...
//     drawOverlay/drawLogo parameter path is exercised; assert no throw.
{
  ...
  let ok2 = true,
    ok3 = true;
  try {
    createRenderer(fakeCanvas, { kind: "2d" }).render(w);
  } catch (e) {
    ok2 = false;
    console.log("2d smoke:", e.message);
  }
  try {
    createRenderer(fakeCanvas, { kind: "3d" }).render(w);
  } catch (e) {
    ok3 = false;
    console.log("3d smoke:", e.message);
  }
...
//     exact corner mapping, parallelogram closure over tiles x heights
{
  ...
  const tN = up(project(5, 5), 24),
    tE = up(project(6, 5), 24),
    tW = up(project(5, 6), 24),
    tS = up(project(6, 6), 24);
  check(
    "worked-example top corners tN/tE/tW/tS",
    tN.sx === 284 &&
      tN.sy === 124 &&
      tE.sx === 304 &&
      tE.sy === 134 &&
  ...
  check(
    "diamondTransform -> {a:.5,b:.25,c:-.5,d:.25,e:284,f:124}",
    m.a === 0.5 &&
      m.b === 0.25 &&
      m.c === -0.5 &&
      m.d === 0.25 &&
  ...
  const ap = (mm, u, v) => ({
    sx: mm.a * u + mm.c * v + mm.e,
    sy: mm.b * u + mm.d * v + mm.f,
  });
  const cN = ap(m, 0, 0),
    cE = ap(m, CFG.TILE, 0),
    cW = ap(m, 0, CFG.TILE),
  ...
  check(
    "source corners land exactly on tN/tE/tW/tS",
    cN.sx === tN.sx &&
      cN.sy === tN.sy &&
      cE.sx === tE.sx &&
      cE.sy === tE.sy &&
  ...
  for (const h of [heightFor(1, true), heightFor(1, false)])
    for (const [tx, ty] of [
      ...
      const n = up(project(tx, ty), h),
        e = up(project(tx + 1, ty), h),
        w2 = up(project(tx, ty + 1), h),
        s2 = up(project(tx + 1, ty + 1), h);
      if (e.sx + w2.sx - s2.sx !== n.sx || e.sy + w2.sy - s2.sy !== n.sy)
        closure = false;
    }
...
// 15) per-biome heights: consumption, ??-fallback, PROJ/freeze invariants
{
  ...
  delete b.hBrick;
  const fb =
    heightFor(1, true) === PROJ.WALL_H && heightFor(1, false) === PROJ.BRICK_H;
  b.hWall = sw;
  b.hBrick = sb;
  ...
  const snap =
    '{"TILE_W":40,"TILE_H":20,"WALL_H":24,"BRICK_H":14,' +
    '"PAD":24,"OFF_X":284,"OFF_Y":48,"canvasW":608,"canvasH":352}';
  check(
    "camera.PROJ byte-snapshot unchanged (spec §4: camera untouched)",
    JSON.stringify(PROJ) === snap,
    JSON.stringify(PROJ),
  );
  check(
    "PROJ and BIOMES frozen",
...
//     entity/block/blade geometry.
{
  ...
  for (const [d, m] of slots) {
    ...
    if (!(fMax < sMin && sMin < uMin)) {
      band = false;
      bad = "depth " + d + ": " + fMax + "<" + sMin + "<" + uMin;
    }
  }
  check(
    "every shadow sorts floor<shadow<upper within its depth slot",
    band,
    bad,
  );
  ...
  check(
    "one shadow per caster (walls+bricks+enemies+bombs+items+players)",
    sh === want,
    sh + " vs " + want,
  );
}
{
  ...
  check(
    "blades/fx-only world casts zero shadows",
