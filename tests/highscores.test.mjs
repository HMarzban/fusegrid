...
check(
  "DEFAULT_SCORES has 10 rows",
  DEFAULT_SCORES.length === 10,
  DEFAULT_SCORES.length,
);
check(
  "DEFAULT_SCORES frozen (array+rows)",
  Object.isFrozen(DEFAULT_SCORES) &&
    DEFAULT_SCORES.every((r) => Object.isFrozen(r)),
);
check(
  "DEFAULT_SCORES rows shaped {s,l,d:'2026-08-23'}",
  DEFAULT_SCORES.every(
    (r) => Number.isFinite(r.s) && Number.isFinite(r.l) && r.d === "2026-08-23",
  ),
);
check(
  "DEFAULT_SCORES sorted by s desc",
...
// ---- loadScores: valid payload round-trip ----
{
  ...
  s.setItem(
    HS_KEY,
    '[{"s":777,"l":2,"d":"2026-01-02"},{"s":55,"l":1,"d":"2026-01-01"}]',
  );
  check(
    "valid stored list loads verbatim",
    eq(loadScores(s), [
      { s: 777, l: 2, d: "2026-01-02" },
      { s: 55, l: 1, d: "2026-01-01" },
    ]),
  );
}
...
// ---- loadScores: no store arg under Node (no window) never throws ----
{
  ...
  check(
    "loadScores(null) headless -> defaults, no throw",
    ok && eq(got, DEFAULT_SCORES),
  );
}
...
// ---- recordScore: sort + trim + tie-break + immutability ----
{
  const base = [];
  for (let i = 0; i < 10; i++)
    base.push({ s: 100 * (10 - i), l: 1, d: "2026-08-23" });
  const inputCopy = JSON.parse(JSON.stringify(base));
  const out = recordScore(base, { s: 950, l: 5, d: "2026-09-01" });
  check(
    "recordScore returns NEW array (input untouched)",
    eq(base, inputCopy) && out !== base,
  );
  check(
    "recordScore inserts into sorted position",
    out[1].s === 950 && out.length === 10,
    "out[1].s=" + out[1].s,
  );
  check(
    "recordScore trims to 10 (lowest dropped)",
    out[out.length - 1].s === 200,
...
}
{
  ...
  check(
    "recordScore keeps sub-10 lists intact",
    out.length === 2 && out[0].s === 60 && out[1].s === 50,
  );
}
{
  const ties = [
    { s: 500, l: 1, d: "2026-03-03" },
    { s: 500, l: 3, d: "2026-01-01" },
    { s: 500, l: 3, d: "2026-05-05" },
    { s: 400, l: 9, d: "2026-01-01" },
  ];
  const out = recordScore(ties, ties[0]);
  check(
    "tie-break: equal s -> higher l first",
    out[0].l === 3 && out[1].l === 3 && out[2].l === 1,
    JSON.stringify(out.slice(0, 3).map((r) => r.l)),
  );
  check(
    "tie-break: equal s+l -> older d first",
...
// ---- qualifies boundary ----
{
  ...
  check(
    "qualifies: short list always true",
    qualifies(0, [...full].slice(0, 3)) === true,
  );
...
// ---- saveScores best-effort: throwing store swallowed ----
{
  ...
  let threw = false;
  try {
    saveScores([{ s: 1, l: 1, d: "2026-08-23" }], boom);
  } catch (e) {
    threw = true;
  }
...
{
  const out = recordScore([{ s: 1, l: 1, d: "2026-08-23" }], {
    s: 2,
    l: 2,
    d: "2026-08-24",
    extra: "junk",
  ...
  check(
    "recordScore copies only {s,l,d}",
    !("extra" in out[0]),
    JSON.stringify(out[0]),
  );
}
{
  const out = recordScore([{ s: 100, l: 1, d: "2026-08-23" }], {
    s: 100,
    l: 1,
    d: "2026-08-24",
    t: 2,
  ...
  check(
    "recordScore keeps heat t, strips extra; MAX ranks above CORE on tie",
    out[0].t === 2 && out[1].t == null && !("extra" in out[0]),
    JSON.stringify(out),
  );
}
